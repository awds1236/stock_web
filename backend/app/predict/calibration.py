...
def out_of_sample_r2(
    ...
    **분모의 기준선이 0(또는 무조건부 평균)입니다.** 인샘플 평균을 기준으로 쓰면
    ...
    횡단면 크기 주의:
        종목 수가 적으면 이 추정치의 분산이 매우 커집니다. 신호가 전혀 없는
        데이터로 확인한 결과, 12종목에서는 R² +0.028 이 우연히 나왔지만 40종목
        -0.002, 120종목 -0.002 로 수렴했습니다. **수십 종목 미만의 유니버스에서
        나온 R² 는 신뢰하지 마십시오.**

    벤치마크 선택:
        기본값 0 은 Gu/Kelly/Xiu 등 개별종목 예측 문헌의 관례입니다(과거 평균은
        개별종목에서 잡음이 크기 때문). 다만 이 기준은 모형이 **무조건부 드리프트를
        맞힌 것만으로도** R² 를 얻게 합니다. 순수한 횡단면 변별력을 보려면
        benchmark 에 실제 평균을 넣어 비교하십시오.
    """
